...
namespace ce {
    CEswapchain::CEswapchain(CEdevice &deviceRef, VkExtent2D windowExtent) : device{deviceRef}, windowExtent{windowExtent} {
        createSwapChain();
        createImageViews();
        createRenderPass();
    ...
    uint32_t CEswapchain::getWidth() {
        return swapChainExtent.width;
    }

    uint32_t CEswapchain::getHeight() {
        return swapChainExtent.height;
    }

    VkRenderPass CEswapchain::getRenderPass() {
        return renderPass;
    }

    VkFormat CEswapchain::getSwapChainImageFormat() {
        return swapChainImageFormat;
    }

    size_t CEswapchain::getImageCount() {
        return swapChainImages.size();
    }

    VkFramebuffer CEswapchain::getFrameBuffer(int index) {
        return swapChainFramebuffers[index];
    }

    VkExtent2D CEswapchain::getSwapChainExtent() {
        return swapChainExtent;
    }

    VkImageView CEswapchain::getImageView(int index) {
        return swapChainImageViews[index];
    }

    float CEswapchain::getExtentAspectRatio() {
        return static_cast<float>(swapChainExtent.width) / static_cast<float>(swapChainExtent.height);
    }

    VkFormat CEswapchain::findDepthFormat() {
    ...
    VkResult CEswapchain::acquireNextImage(uint32_t *imageIndex) {
    ...
    VkResult CEswapchain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex) {
    ...
    void CEswapchain::createSwapChain() {
    ...
    void CEswapchain::createImageViews() {
    ...
    void CEswapchain::createRenderPass() {
    ...
    void CEswapchain::createDepthResources() {
    ...
    void CEswapchain::createFramebuffers() {
    ...
    void CEswapchain::createSyncObjects() {
    ...
    VkPresentModeKHR CEswapchain::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes) { //TO-DO: opt-in: VK_PRESENT_MODE_IMMEDIATE_KHR (no V-Sync)
    ...
    VkExtent2D CEswapchain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
